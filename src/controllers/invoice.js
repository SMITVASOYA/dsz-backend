...
const retrieveInvoicesOfQuery = async (req, res) => {
  try {
    ...
    invoices.forEach((invoice, index) => {
      ...
      const invoice_data = JSON.parse(invoice.dataValues.invoice_data)
      console.log(
        '🚀 ~ file: invoice.js:266 ~ invoices.forEach ~ invoice_data:',
        invoice_data
      )
      const generatedInvoiceNumber = buildInvoiceNumber(
        invoice.dataValues.invoice_number,
      ...
      invoices[index].dataValues.generatedInvoiceNumber = generatedInvoiceNumber
    })
    return res.status(200).json({
      error: false,
      data: invoices,
