...
const leaveNotificationToHR = async (req, res) => {
  try {
    ...
    if (payload.leave_req_start_date > payload.leave_req_end_date)
      throw new Error('Start Date must be less than end Date')
    console.log(req.session.id, 360)
    // commenting because it is generating error for employee
...
const getAllLeavesHistory = async (req, res) => {
  try {
    const leaveRequests = await db.leave_req.findAll({
      where: {
        leave_req_status: { [Op.ne]: 'new' },
      },
      include: {
        model: db.employee,
        as: 'employee',
...
const updateLeaveReqStatus = async (req, res) => {
  try {
    ...
    let payload = {
      leave_req_status: req.body.status,
    }
    if (req.body.status === 'rejected')
      payload.leave_req_reject_reason = req.body.message
...
const getAllLeavesOfEmlpoyee = async (req, res) => {
  try {
    const leaveRequestsOfEmployee = await db.leave_req.findAll({
      where: {
        employee_id: req.params.employee_id,
      },
      include: {
